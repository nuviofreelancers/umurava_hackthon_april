...
// GET /api/results
// BUG FIX #3: always scope results to the requesting user's jobs — this prevents
//             candidates from other users appearing on the dashboard.
export const getResults = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Collect all job IDs owned by this user so we can always scope the query
    const userJobIds = await Job.find({ userId: req.user!.id }).distinct("_id");

    const filter: any = { job_id: { $in: userJobIds } }; // BUG FIX #3: base filter scoped to user

    if (jobIdParam && jobIdParam !== "undefined" && mongoose.Types.ObjectId.isValid(jobIdParam)) {
      // Verify the requested job belongs to this user
